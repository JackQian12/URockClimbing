...
| 当前阶段 | MVP / V1 |
| 核心目标 | 跑通会员注册、次卡/期限卡购买、会员卡查看、到店核销闭环 |
...
V1 只实现会员、次卡/期限卡、购买、核销和记录查询，不提前实现未来功能，但数据库主键、订单模型、权限模型和模块划分不得阻碍扩展。
...
2. 用户可以查看并购买在售次卡、周卡、月卡、季卡和年卡。
3. 微信支付成功后，会员卡自动到账且不可重复发放。
4. 用户到店后可以展示一次性核销码。
5. 管理员可以生成可打印、可重复扫描的场馆签到码；会员到店后主动扫码完成签到或次数核销。
6. 员工可扫码核销，系统准确扣减一次并留下审计记录。
7. 管理员可管理会员卡商品，查看会员、订单、核销数据，并在后台对符合条件的订单发起退款。

### 2.2 MVP 成功标准

- 新用户在 1 分钟内完成登录并进入首页。
- 支付回调成功后 10 秒内在“我的会员卡”看到新卡。
- 正常网络下，核销操作 3 秒内返回明确结果。
- 同一一次性核销码，或同一幂等确认请求并发提交时，最多产生一条业务流水。
- 每次余额变化均有不可覆盖的业务流水。
...
| 管理员 | `ADMIN` | 员工全部权限；管理会员卡商品；查询全部会员、订单、卡和核销记录；对符合条件的订单发起全额退款；设置员工角色 |
...
1. **会员定义**：微信登录只建立待注册账号和会话；用户主动授权手机号且后端成功绑定后，以 `registered_at` 为准完成会员注册，不另外收取会员费。
2. **会员卡类型**：`COUNT_CARD` 为固定次数的次卡，一次核销扣减 1 次；`TIME_PASS` 为期限卡，有效期内不限总次数且每日最多核销 1 次。
3. **期限卡规格**：周卡 7 天、月卡 30 天、季卡 90 天、年卡 365 天；仅限本人使用，不支持分享。
4. **生效时间**：期限卡支付成功后为 `PENDING_ACTIVATION`，首次核销时激活；次卡按商品的 `activation_mode` 生效。
5. **过期时间**：激活后的卡片以 `activated_at + validity_days` 计算 `expires_at`，精确到秒；到期时刻之后不可核销。
6. **使用顺序**：用户主动选择某一张卡生成核销码，不自动从多张卡中扣除。
7. **退款**：会员端不提供退款按钮或退款 API。退款只能由 `ADMIN` 在管理后台发起。V1 仅支持整单全额退款，且该订单发放的会员卡必须从未核销；部分退款和已核销订单退款不支持自动处理。
8. **删除**：商品、订单、会员卡、核销记录均不得物理删除。商品使用上下架状态。
9. **金额**：所有金额以人民币分为整数存储和计算，禁止使用浮点数。
10. **时间**：数据库存 UTC；API 使用 RFC3339；前端以 Asia/Shanghai 展示。
11. **手机号**：V1 必须经用户明确授权才能完成注册，但不作为登录凭证。小程序只将 `getPhoneNumber` 动态 `code` 传给后端，由后端向微信换取平台验证过的号码；`code` 不存储、不记日志。
12. **昵称和头像**：不得在登录时静默获取微信资料。用户可主动授权获取微信昵称，或通过微信昵称输入组件选择后保存；未授权时使用“岩友”和本地默认头像。首页登录后优先显示已保存的昵称。
13. **订单快照**：商品改名、改价、下架不能影响历史订单和已发放会员卡。
...
5. 后端按 `openid + appid` 查找身份；不存在则创建待注册用户和会员档案，`registered_at` 为空。
6. 后端签发访问令牌和刷新令牌。
7. 未注册用户点击 `open-type="getPhoneNumber"` 按钮并明确同意授权。
8. 前端将一次性动态 `code` 提交给 `POST /me/phone`，后端向微信换取手机号。
9. 后端加密保存完整号码，写入后四位、`phone_authorized_at` 和 `registered_at`，授权即注册完成。
10. 未完成注册的账号不得购卡、查看会员权益、生成核销码或使用员工功能。
11. 用户可在会员中心主动选择微信昵称并保存；小程序首页使用该昵称生成问候语。

### 5.2 购买会员卡

1. 用户进入“会员卡商城”，查看在售次卡和期限卡。
2. 用户选择商品并确认购买。
...
1. 用户进入“我的会员卡”，选择一张可用的 `ACTIVE` 或 `PENDING_ACTIVATION` 卡。
2. 用户点击“出示核销码”。
3. 后端校验卡片所有权、状态、有效期；次卡额外校验剩余次数，期限卡额外校验当日使用次数。
...
3. 前端向后端请求核销预览。
4. 后端返回会员脱敏信息、卡名称、激活/到期时间和次数权益。
5. 员工点击确认核销。
6. 后端在数据库事务中锁定会员卡和核销令牌，再次校验全部条件。
7. 若为待激活期限卡，在事务内设置 `activated_at`、`expires_at` 并转为 `ACTIVE`；若为次卡则扣减 1 次。
8. 写入核销流水并将令牌标记为已使用；期限卡流水的次数余额字段为空。
9. 返回核销成功页面，展示权益变化和流水号。
10. 重复提交同一请求应返回第一次的核销结果，不得再次记录或扣减。
...
1. 管理员在 Web 后台创建场馆签到码，填写便于识别的名称，例如“前台签到码”。
2. 系统生成不可猜测的随机令牌和二维码；二维码可下载或直接打印，并可被多名会员重复扫描。
3. 管理员可启用、停用或换码。停用后旧二维码立即不可用；换码后旧二维码立即不可用。
4. 会员在小程序点击“扫码签到/核销”，扫描场馆签到码。
5. 后端校验签到码为启用状态，返回该会员当前可使用的会员卡，会员必须主动选择一张卡。
6. 次卡确认后扣减 1 次；期限卡确认后记录当日签到，不扣减次数，首次签到时激活并计算有效期。
7. 期限卡仍遵守商品的每日使用上限；V1 期限卡同一自然日最多签到 1 次。
8. 确认操作必须使用客户端幂等键，并在事务中锁定签到码和会员卡；重复提交只能产生一条核销流水。
9. 场馆签到码长期有效，不因一次扫描而失效；有效性仅由后台状态和换码操作控制。
10. 场馆签到码泄露后可能被异地扫描，管理员应立即停用或换码；系统保留会员、签到码、时间和请求 IP 审计信息。
...
1. **首页**：会员概要、可用会员卡、快捷购卡、最近核销。
2. **会员卡**：在售次卡和期限卡商品列表、我的会员卡切换入口。
...
| 会员卡商城 | `pages/card-store/index` | 在售商品、权益类型、有效期、价格 | 全部 |
| 会员卡详情 | `pages/card-product/detail` | 商品详情、使用规则、购买按钮 | 全部 |
| 我的会员卡 | `pages/my-cards/index` | 待激活、可用、已用完、已过期分类 | 全部 |
| 会员卡详情 | `pages/my-cards/detail` | 次数/期限权益、核销记录、出示核销码 | 全部 |
| 核销码 | `pages/redemption-code/index` | 动态二维码、倒计时、自动刷新提示 | 全部 |
| 扫码签到/核销 | `pages/checkin/index` | 扫描场馆码、选择会员卡、确认签到或扣次 | 全部 |
...
| 会员卡商品 | `/card-products` | 新增、编辑、上下架、版本冲突提示 | ADMIN |
...
| 签到码管理 | `/checkin-codes` | 创建、启停、换码、打印场馆签到码 | ADMIN |
...
### 7.3 用户会员卡 `member_card.status`

```text
PENDING_ACTIVATION -> ACTIVE -> USED_UP
   |
   -> EXPIRED
   |
...
- `PENDING_ACTIVATION`：已发卡但尚未首次核销，`activated_at` 和 `expires_at` 为空。
- `ACTIVE`：已生效且未过期；次卡的剩余次数同时必须大于 0。
- `USED_UP`：次卡剩余次数为 0；期限卡不使用此状态。
- `EXPIRED`：到达过期时间。
...
| `registered_at` | datetime(3) | 手机号绑定成功即写入；为空表示待注册 |
...
| `product_type` | varchar(16) | COUNT_CARD/TIME_PASS |
| `total_times` | int | 次卡大于 0；期限卡为空 |
...
| `product_type` | varchar(16) | COUNT_CARD/TIME_PASS 快照 |
| `total_times` | int | 次卡初始总次数；期限卡为空 |
| `remaining_times` | int | 次卡为 0 到 total_times；期限卡为空 |
| `status` | varchar(24) | PENDING_ACTIVATION/ACTIVE/USED_UP/EXPIRED/REFUND_LOCKED/REFUNDED |
| `activated_at` | datetime(3) | 生效时间，待激活时为空 |
| `expires_at` | datetime(3) | 到期时间，待激活时为空 |
| `version` | int | 并发控制 |
...
| 字段 | 类型建议 | 约束/说明 |
|---|---|---|
| `code_no` | varchar(64) | 全局唯一、后台展示 |
| `name` | varchar(100) | 张贴位置或场馆码名称 |
| `token_hash` | char(64) | 唯一；用于扫码校验，不存原文 |
| `token_encrypted` | varbinary(512) | 加密保存，仅用于管理员重新打印 |
| `status` | varchar(16) | ACTIVE/INACTIVE |
| `version` | int | 启停和换码的乐观锁 |
| `created_by` | FK | 创建该码的 ADMIN |

签到码长期可重复使用，不得在单次签到后修改其状态或令牌；只有管理员停用或换码时才失效。

### 8.9 `redemption_records`
...
| `user_id` | FK | 会员 |
| `operator_user_id` | FK | 员工扫码时为核销员工；会员自助时为会员本人 |
| `token_id` | FK | 员工扫码时填写且唯一；会员自助时为空 |
| `checkin_code_id` | FK | 会员自助扫码时关联可复用签到码；员工扫码时为空 |
| `redemption_mode` | varchar(24) | STAFF_SCAN/MEMBER_SELF_SCAN |
| `times` | int | V1 固定为 1 |
| `before_remaining` | int | 次卡核销前次数；期限卡为空 |
| `after_remaining` | int | 次卡核销后次数；期限卡为空 |
| `redeemed_at` | datetime(3) | 核销时间 |
| `request_id` | varchar(64) | 确认操作幂等键，与 operator 组成唯一索引 |

### 8.10 `audit_logs`

记录管理员修改商品、角色，以及关键异常人工操作。字段至少包含：操作者、动作、资源类型、资源 ID、变更前后摘要、请求 IP、请求 ID、时间。

### 8.11 `refund_transactions`
...
| POST | `/me/phone` | 消费微信手机号动态 code，绑定号码并完成注册 | 已登录待注册账号 |
...
| POST | `/me/checkins/preview` | 校验场馆签到码并返回本人可用会员卡 | MEMBER |
| POST | `/me/checkins/confirm` | 会员选择卡后确认签到或扣次 | MEMBER |

### 9.5.1 场馆签到码管理

| 方法 | 路径 | 功能 | 权限 |
|---|---|---|---|
| GET/POST | `/admin/checkin-codes` | 查询/创建场馆签到码 | ADMIN |
| GET | `/admin/checkin-codes/{id}` | 获取可打印二维码内容 | ADMIN |
| PUT | `/admin/checkin-codes/{id}/status` | 启用或停用签到码（携带版本号） | ADMIN |
| POST | `/admin/checkin-codes/{id}/regenerate` | 换码并立即废止旧二维码（携带版本号） | ADMIN |
...
5. 再次校验卡状态与过期时间；次卡校验 `remaining_times > 0`，期限卡校验当日核销数小于 1。
6. 次卡将剩余次数减 1，并在为 0 时置为 `USED_UP`；期限卡若为 `PENDING_ACTIVATION`，在此事务中激活并计算到期时间。
...
- 首次微信登录和重复登录；
- 手机号授权 code 失效/重复消费、重复注册幂等；
- 未完成手机号注册的账号不能访问会员业务接口；
- 禁用用户登录；
- 商品上下架与下架商品禁止下单；
- 创建订单幂等；
- 支付回调验签失败；
...
4. 会员必须授权手机号才完成注册，并且只有已注册会员可以购卡。
