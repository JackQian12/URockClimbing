...
function cardBalance(order: AdminOrder) { return order.card_product_type === 'TIME_PASS' ? `每日 1 次 · ${order.card_status}` : `${order.card_remaining_times}/${order.card_total_times} 次 · ${order.card_status}` }
...
export function OrdersPage() {
  ...
  async function submitRefund(event: FormEvent) {
    ...
    if (utf8Length(refundReason.trim()) > 80) { setRefundError('退款原因不得超过 80 字节'); return }
  ...
  async function syncRefund(order: AdminOrder) {
    if (!order.refund_no || syncingRefund) return
    setSyncingRefund(true); setError('')
    try {
      await apiRequest(`/admin/refunds/${order.refund_no}/sync`, { method: 'POST' })
      setSelected(null); setNotice('已从微信支付同步退款状态'); await load()
    } catch (reason) { setError(reason instanceof Error ? reason.message : '退款状态同步失败') }
    finally { setSyncingRefund(false) }
  }
  ...
  return <div className="page-content order-page">
    ...
    {!loading&&!error&&orders.length>0&&<section className="member-table-card"><div className="member-table-wrap"><table className="member-table order-table"><thead><tr><th>订单 / 商品</th><th>会员</th><th>金额</th><th>支付</th><th>次卡</th><th>下单时间</th><th>状态</th><th/></tr></thead><tbody>{orders.map(order=>{const status=statusMeta[order.status];return <tr key={order.id}>
      <td><strong className="table-primary">{productName(order)}</strong><small>{order.order_no}</small></td><td><strong className="table-primary">{memberName(order)}</strong><small>{order.member_no} · {order.member_phone||'未授权手机'}</small></td><td><strong className="table-primary">{money(order.paid_amount_cent||order.total_amount_cent)}</strong><small>{order.paid_amount_cent?'实付':'应付'}</small></td><td><strong className="table-primary">{order.payment_status||'未创建'}</strong><small>{order.provider_transaction_id?'微信交易已确认':'暂无微信交易号'}</small></td><td><strong className="table-primary">{order.card_no||'未发卡'}</strong><small>{order.card_no?cardBalance(order):'—'}</small></td><td><strong className="table-primary">{dateTime(order.created_at)}</strong><small>{order.paid_at?`支付 ${dateTime(order.paid_at)}`:'尚未支付'}</small></td><td><span className={`order-status ${status.className}`}>{status.label}</span>{order.refund_status&&<small>退款单：{order.refund_status}</small>}</td><td><button className="detail-button" onClick={()=>setSelected(order)}>查看</button></td>
    ...
    {selected&&<div className="modal-backdrop member-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)setSelected(null)}}><section className="member-drawer order-drawer" role="dialog" aria-modal="true"><header><div><p className="eyebrow">ORDER DETAIL</p><h2>{productName(selected)}</h2><small>{selected.order_no}</small></div><button className="modal-close" onClick={()=>setSelected(null)}>×</button></header><div className="member-detail-body">
      ...
      <section className={`refund-eligibility ${selected.refund_eligible?'eligible':'ineligible'}`}><strong>{selected.refund_eligible?'可以申请全额退款':'当前不可退款'}</strong><p>{selected.refund_eligible?'会员卡尚未核销，退款后将立即锁定权益，等待微信最终结果。':selected.refund_ineligible_reason}</p></section>
    </div><footer><button className="secondary-button compact-secondary" onClick={()=>setSelected(null)}>关闭</button>{selected.refund_no&&['CREATED','PROCESSING','ABNORMAL'].includes(selected.refund_status||'')&&<button className="secondary-button compact-secondary" disabled={syncingRefund} onClick={()=>void syncRefund(selected)}>{syncingRefund?'同步中…':'同步退款状态'}</button>}{selected.refund_eligible&&<button className="danger-solid" onClick={()=>{setRefundReason('');setRefundError('');setRefundOrder(selected)}}>全额退款</button>}</footer></section></div>}

    {refundOrder&&<div className="modal-backdrop refund-backdrop"><section className="refund-modal" role="dialog" aria-modal="true"><header><div><p className="eyebrow">FULL REFUND</p><h2>确认全额退款</h2></div><button className="modal-close" onClick={()=>setRefundOrder(null)}>×</button></header><form onSubmit={submitRefund}><div className="refund-amount"><span>原路退回</span><strong>{money(refundOrder.paid_amount_cent)}</strong><small>{memberName(refundOrder)} · {refundOrder.order_no}</small></div><label>退款原因<textarea rows={4} maxLength={80} value={refundReason} onChange={e=>setRefundReason(e.target.value)} placeholder="例如：顾客误购，会员卡尚未使用"/><small>{utf8Length(refundReason)} / 80 字节，该原因将提交至微信支付</small></label><div className="refund-warning"><strong>退款安全检查</strong><p>仅支持整单全额退款。提交后会员卡会锁定；微信受理不等于退款成功，需等待最终状态。</p></div>{refundError&&<div className="form-error">{refundError}</div>}<footer><button type="button" className="secondary-button compact-secondary" disabled={refunding} onClick={()=>setRefundOrder(null)}>取消</button><button className="danger-solid" disabled={refunding}>{refunding?'提交中…':'确认全额退款'}</button></footer></form></section></div>}
  </div>
}
